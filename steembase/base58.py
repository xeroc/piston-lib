...
class Base58(object) :
    ...
    :param str prefix: Prefix to use for Address/PubKey strings (defaults to ``STM``)
    ...
    *  ``format(Base58,_format)`` Formats the instance according to ``_format``:
    ...
    def __init__(self, data, prefix=PREFIX) :
        ...
        if all(c in string.hexdigits for c in data) :
            ...
            self._hex     = steemBase58CheckDecode(data[len(self._prefix):])
        else :
            raise ValueError("Error loading Base58 object")

    def __format__(self, _format) :
        """ Format output according to argument _format (wif,btc,stm)
    ...
    def __str__(self) :
        """ Return base58CheckEncoded string of data
            ...
            :rtype: str
        """
        return steemBase58CheckEncode(self._hex)
...
def steemBase58CheckEncode(s):
    checksum = ripemd160(s)[:4]
    result = s + hexlify(checksum).decode('ascii')
    return base58encode(result)


def steemBase58CheckDecode(s):
    s   = unhexlify(base58decode(s))
    dec = hexlify(s[:-4]).decode('ascii')
